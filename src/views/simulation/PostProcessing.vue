...
const checkExeStatus = async () => {
  try {
    ...
    statusMessage.value = "无法连接到服务器，程序可能已停止"
...
<template>
  <div>
    ...
    <div ref="container" class="exe-container" :class="{ active: isExeRunning }">
      <div v-if="!isExeRunning" class="placeholder-message">
        {{ taskType ? `检测到任务类型: ${taskType}，点击"启动程序"按钮加载应用` : '点击"启动程序"按钮加载内嵌应用' }}
      </div>
